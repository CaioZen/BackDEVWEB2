...
public class ClasseService {
    ...
    public Classe buscarClassePorId(UUID id) {
        return classeRepository.findById(id).orElseThrow(() -> new RecursoNaoEncontradoException("Classe não encontrada"));
    }
    ...
    public Classe atualizarClasse(UUID id, Classe classeAtualizada) {
        return classeRepository.findById(id).map(classe -> {
            ...
            return classeRepository.save(classe);
        }).orElseThrow(() -> new RecursoNaoEncontradoException("Classe não encontrada"));
    }

    public void deletarClasse(UUID id) {
        if (!classeRepository.existsById(id)) {
            throw new RecursoNaoEncontradoException("Classe não encontrada");
        }
